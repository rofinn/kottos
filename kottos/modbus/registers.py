...
RESTING_REASONS = {
    ...
    9: "Too much negative current while operating "
       "(backfeed from battery out of PV input)",
    10: "Battery is less than 8.0 Volts",
    11: "PV input is available but V is rising too slowly. Low Light or bad connection "
        "(Solar mode)",
    ...
    26: "Mode changed while running OR Vabsorb raised more than 10.0 Volts at once OR "
        "Nominal Vbatt changed by modbus command AND MpptMode was ON when changed",
    27: "bridge center == 1023 (R132 might have been stuffed) "
        "This turns MPPT Mode to OFF",
    28: "NOT Resting but RELAY is not engaged for some reason",
    29: "ON/OFF stays off because WIND GRAPH is illegal "
        "(current step is set for > 100 amps)",
    30: "PkAmpsOverLimit... Software detected too high of PEAK output current",
    31: "AD1CH.IbattMinus > 900 Peak negative battery current > 90.0 amps "
        "(Classic 250)",
    32: "Aux 2 input commanded Classic off. for HI or LO (Aux2Function == 15 or 16)",
    33: "OCP in a mode other than Solar or PV-Uset",
    34: "AD1CH.IbattMinus > 900 Peak negative battery current > 90.0 amps "
        "(Classic 150, 200)",
    35: "Battery voltage is less than Low Battery Disconnect (LBD) Typically Vbatt is "
        "less than 8.5 volts",
...
MNS_REGISTER_TABLE = [
    Register(
        name="Resting Reason", reg=4275, func=lambda x: RESTING_REASONS[x], units=None
    ),
    Register(
        name="Battery Voltage", reg=4115, func=lambda x: float(x) / 10.0, units="V"
    ),
    Register(name="Input Voltage", reg=4116, func=lambda x: float(x) / 10.0, units="V"),
    Register(
        name="Battery Current", reg=4117, func=lambda x: float(x) / 1.0, units="A"
    ),
    Register(
        name="Batter Energy", reg=4118, func=lambda x: float(x) / 10.0, units="kWh"
    ),
    Register(name="Battery Power", reg=4119, func=lambda x: float(x), units="W"),
    Register(
        name="Charge State", reg=4120, func=lambda x: CHARGE_STATES[x >> 8], units=None
    ),
    Register(name="Input Current", reg=4121, func=lambda x: float(x) / 10.0, units="A"),
    Register(
    ...
    Register(name="Amp Hours", reg=4125, func=lambda x: float(x), units="Ah"),
    Register(name="Battery Temp", reg=4132, func=lambda x: float(x) / 10.0, units="°C"),
    Register(name="FET Temp", reg=4133, func=lambda x: float(x) / 10.0, units="°C"),
    Register(name="PCB Temp", reg=4134, func=lambda x: float(x) / 10.0, units="°C"),
]
